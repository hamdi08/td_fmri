function tensor_subject_rois_ts_adhd

clear; clc; close all; dbstop if error;
rng(3);

%cd /home/hamdi/fmri_studies/ADHD_200/AAL_TS/data;
load('subjects_768_adhd.mat');
...
for i=1:allSubjects
        ...
        disp('Tensor entry:')
        disp(count);
        fmri_tensor(count, :, :) = subject_db{83 + count, 3}; %first 83 are KKI
        y(count) = subject_db{83 + count, 10}; %binary; case 2
end
...
ylabel('Accuracy');
title('Classification accuracy with SVM and stratified 8 fold cross validation');
...
for i=1:allSubjects
        ...
        disp('Tensor entry:')
        disp(count);
        func_mat = subject_db{83 + count, 4};
        func_mat(isnan(func_mat)) = [1];
        fmri_tensor(count, :, :) = func_mat; %first 83 are KKI
        y(count) = subject_db{83 + count, 10}; %binary; case 2
end
...
for i=1:allSubjects
        ...
        disp('Tensor entry:')
        disp(count);
        fmri_tensor(count, :, :) = subject_db{83 + count, 5}; %first 83 are KKI
        y(count) = subject_db{83 + count, 10}; %binary; case 2
end
